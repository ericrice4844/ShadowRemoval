...
// ##################################################################################################
// ###   serialHSVConverter()   ###
// This is a serial implementation of the RGB to HSV converter
void LrTextureShadRem::serialHSVConverter(const cv::Mat &frame, unsigned char frameChar[IM_ROWS][IM_COLS][IM_CHAN])
{
	for (int ii = 0; ii < IM_ROWS; ii++)
	{
		for (int jj = 0; jj < IM_COLS; jj++)
		{
			...
			for (int kk = 0; kk < IM_CHAN; kk++)
			{
				...
				if (kk == 0)
				{
					R = (unsigned char)(frame.at<Vec3b>(ii, jj)[kk]);
				}
				else if (kk == 1)
				{
					G = (unsigned char)(frame.at<Vec3b>(ii, jj)[kk]);
				}
				else
				{
					B = (unsigned char)(frame.at<Vec3b>(ii, jj)[kk]);
				}
			}
			...
			// Determine the largest and smallest value between each of the three channels
			if ((R > G) && (R > B))
			{
				Cmax = R;
				if (G >= B)
			...
			// Write values
			frameChar[ii][jj][0] = H / 2;
			frameChar[ii][jj][1] = S * 255;
			frameChar[ii][jj][2] = V * 255;
		}
	}
}
...
// ##################################################################################################
// ###   serialGrayConverter()   ###
// This is the serial implementation of the RGB to grayscale converter
void LrTextureShadRem::serialGrayConverter(const cv::Mat &frame, unsigned char frameChar[IM_ROWS][IM_COLS * IM_CHAN])
{
	for (int ii = 0; ii < IM_ROWS; ii++)
	{
		for (int jj = 0; jj < IM_COLS; jj++)
		{
			...
			for (int kk = 0; kk < IM_CHAN; kk++)
			{
				if (kk == 0)
				{
					R = (unsigned char)((frame.at<Vec3b>(ii, jj)[kk]) * 0.21f);
				}
				else if (kk == 1)
				{
					G = (unsigned char)((frame.at<Vec3b>(ii, jj)[kk]) * 0.71f);
				}
				else
				{
					B = (unsigned char)((frame.at<Vec3b>(ii, jj)[kk]) * 0.07f);
				}
			}
			float total = R + G + B;
			frameChar[ii][jj] = total;
		}
	}
}
...
void LrTextureShadRem::removeShadows(const cv::Mat &frame, const cv::Mat &fgMask, const cv::Mat &bg, cv::Mat &srMask)
{
	...
	//auto frameHSVChar = new unsigned char[IM_ROWS][IM_COLS * IM_CHAN]{};
	...
	if (use_cuda)
	{
		// Call conversion for Grayscale and HSV
		convertRGBToGrayAndHSV(frame, grayFrame, hsvFrame);
...
float LrTextureShadRem::getGradDirCorr(const cv::Mat &grayFrame, const ConnComp &cc, const cv::Mat &grayBg)
{
	...
	gradCorrThresh = (avgAtten < params.avgAttenThresh ? params.gradCorrThreshLowAtten : params.gradCorrThreshHighAtten);
